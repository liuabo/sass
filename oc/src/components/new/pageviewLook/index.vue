...
export default {
  ...
  data() {
    return {
      ...
      topTitle:'阅读数据',
      Loading:false,
      personClass:[{id:1,name:'阅读人员'},{id:2,name:'未读人员'},{id:3,name:'点赞人员'}],
      list:[],
      listLength:'',
  ...
  methods:{
      getReadList(){//已读
          ...
          this.$api.post("web/newsRead/readList", data, r => {
      ...
      getUnreadList(){//未读
          ...
          this.$util.hintShow(this,'加载中...')
          this.$api.post("web/newsRead/unreadList", data, r => {
              this.list = r.data.rows;
            this.listLength= r.data.total
             this.$util.hintHide(this)
          });
      ...
      getList(){//点赞
          ...
          this.$util.hintShow(this,'加载中...')
          this.$api.post("web/newsLike/list", data, r => {
            this.list = r.data.rows;
            this.listLength= r.data.total
            this.$util.hintHide(this)
          });
        ...
        if(scrollHeight-clientHeight - scrollTop <=  45){
            if(this.onOff){
              if(this.off){
                  ...
                  let url = ''
                  if(this.shouClasses == 1){
                      url = 'web/newsRead/readList'
                  }else if(this.shouClasses == 2){
                    url = 'web/newsRead/unreadList'
                  }else if(this.shouClasses == 3){
                    url = 'web/newsLike/list'
                  }
                  let data={
                      limit:this.limit,
                      offset:this.page * this.limit,
      ...
      derivedTable(){//导出
          let url = ''
          if(this.shouClasses == 1){
              url =  '/web/newsRead/exportReadUserInfo'
          }else if(this.shouClasses == 2){
              url = '/web/newsRead/exportUnReadUserInfo'
          }else if(this.shouClasses == 3){
              url = '/web/newsLike/exportNewsLikeInfo'
          }
           let config = {
      ...
      axios.post(this.$newsUrl +  url , 'newsId='+ this.newsId ,config)
        .then(res => {
      ...
      remindUnReadUser(){//未读人员提醒
        let data = {
          newsId:this.newsId
        }
        this.$util.hintShow(this)
        this.$api.post("web/newsRead/remindUnReadUser", data, r => {
                setTimeout(() => {
                  this.$util.hintHide(this)
                }, 500);
        }); 
