...
export default {
    ...
    props:[ 'coTitle', 'saveUrl' , 'updateUrl', 'getUrl' , 'infoUrl' , 'classiifyUrl','path'],
    data(){
        return{
    ...
    methods:{
        ...
            let config = {headers:{'Content-Type':'multipart/form-data',"Authorization":token}}
           ...
           this.param.set('corpId',sessionStorage['corpId']);
            let url = this.saveUrl ? this.saveUrl : this.$newsUrl + '/omweb/newsTemplate/save'
            let id = this.$route.params.id
            if(id!=0){
                url = this.updateUrl ? this.updateUrl : this.$newsUrl + "/omweb/newsTemplate/update"
            ...
            axios.post(url,this.param ,config)
                .then((r)=>{
                    if(boo){
                        this.$router.push(this.path ? this.path : '/app/newsIndex/contentTemplate')
    ...
        },
        getCategory(){//获取分类类型
            this.$api.post(this.classiifyUrl ? this.classiifyUrl : 'omweb/newsCategory/getLists', null, r => {
            // console.log(r)
                this.categoryList = r.data
            })
        },
        getNewsSource(){//获取信息来源
            this.$api.post(this.infoUrl ? this.infoUrl : 'omweb/newsSource/getLists', null, r => {
